...
const { MsgDeposit } = Root.terra;
MsgDeposit.prototype.type = Config.terra.tx.deposit.prefix;
MsgDeposit.prototype.GetSignBytes = function () {
  const depositor = BECH32.encode(Config.terra.bech32.accAddr, this.depositor);
  const signMsg = {
...
const { MsgVote } = Root.terra;
MsgVote.prototype.type = Config.terra.tx.vote.prefix;
MsgVote.prototype.GetSignBytes = function () {
  const voter = BECH32.encode(Config.terra.bech32.accAddr, this.voter);
  const signMsg = {
    proposal_id: `${this.proposalID}`,
    voter,
