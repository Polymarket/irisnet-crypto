...
const { StdFee } = Root.terra;
const { Coin } = Root.terra;
...
class StdTx {
  ...
  SetSignature(sig) {
    ...
    const signature = new Root.terra.StdSignature({
      pubKey: sig.pub_key,
  ...
  SetPubKey(pubkey) {
    if (Codec.Bech32.isBech32(Config.terra.bech32.accPub, pubkey)) {
  ...
  GetData() {
    ...
    if (this.signatures) {
      this.signatures.forEach((sig) => {
        ...
        signatures.push({
          pub_key: Amino.MarshalJSON(Config.terra.amino.pubKey, publicKey),
          signature,
        });
      });
    }
