...
const { MsgDeposit } = Root.irisnet.tx;
MsgDeposit.prototype.type = Config.iris.tx.deposit.prefix;
MsgDeposit.prototype.GetSignBytes = function () {
  const depositor = BECH32.encode(Config.iris.bech32.accAddr, this.depositor);
  const signMsg = {
...
const { MsgVote } = Root.irisnet.tx;
MsgVote.prototype.type = Config.iris.tx.vote.prefix;
MsgVote.prototype.GetSignBytes = function () {
  const voter = BECH32.encode(Config.iris.bech32.accAddr, this.voter);
  const signMsg = {
    proposal_id: `${this.proposalID}`,
    voter,
