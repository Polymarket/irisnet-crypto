...
class TxSerializer {
  ...
  static encode(object) {
    ...
    const { fee } = object;
    const { StdFee } = root.irisnet.tx;
    const feeMsg = StdFee.create(fee);


    const { StdSignature } = root.irisnet.tx;
    let signature;
    ...
    const { StdTx } = root.irisnet.tx;
    const tx = StdTx.create({
      msgs: [msgBytes], fee: feeMsg, signatures: signature, memo,
    });
    const txMsgBuf = StdTx.encode(tx).finish();

    // stdTx amion编码前缀[auth/StdTx]
    const txPreBuf = Buffer.from(amino.GetRegisterInfo(config.iris.tx.stdTx.prefix).prefix);
