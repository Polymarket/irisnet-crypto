...
const { MsgDeposit } = Root.cosmos;
MsgDeposit.prototype.type = Config.cosmos.tx.deposit.prefix;
MsgDeposit.prototype.GetSignBytes = function () {
  const depositor = BECH32.encode(Config.cosmos.bech32.accAddr, this.depositor);
  const signMsg = {
...
const { MsgVote } = Root.cosmos;
MsgVote.prototype.type = Config.cosmos.tx.vote.prefix;
MsgVote.prototype.GetSignBytes = function () {
  const voter = BECH32.encode(Config.cosmos.bech32.accAddr, this.voter);
  const signMsg = {
    proposal_id: `${this.proposalID}`,
    voter,
